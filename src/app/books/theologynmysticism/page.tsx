...
export default function TheologyMysticismPage() {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        setIsLoading(true)
        const response = await bookApi.getBooksByCategory("Theologynmysticism")
        if (response.success) {
  ...
  return (
    <div className="bg-amber-50 min-h-screen" style={{ background: "url(/images/background.webp)" }}>
      <Head>
        ...
        <meta name="keywords" content="islamic books, theology books, mysticism, islamic mysticism, sufism, quranic philosophy" />
