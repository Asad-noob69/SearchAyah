            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 justify-items-center gap-y-8">
              {row.map((book) => (
                <article key={book.id} className="relative flex flex-col items-center" itemScope itemType="https://schema.org/Book">
                  ...
                  <Link href={`/books/philosophy/${book.slug}`} className="relative block w-28 sm:w-32 md:w-36 lg:w-40 h-40 sm:h-48 md:h-56 lg:h-60">
                    <Image
...
export default function PhilosophyBooksPage() {
  ...
  const { books, filteredBooks, isLoading, error, searchBooks } = useCategoryBooks("Philosophy")
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center">
            ...
            {filteredBooks.length > 0 ? (
              <BookGrid books={filteredBooks} title="Philosophy Books" />
            ) : (
