...
import { Book, BookCategory, islamicJurisprudenceCategory } from "@/lib/book-data"

export default function IslamicJurisprudenceBooksPage() {
  const jurisCategory: BookCategory = islamicJurisprudenceCategory()
  ...
  useEffect(() => {
    if (searchInput) {
      const filtered = jurisCategory.books.filter((book) =>
        book.title.toLowerCase().includes(searchInput.toLowerCase())
      )
      setFilteredBooks(filtered)
      setShowMainContent(filtered.length === 0)
    } else {
      setFilteredBooks(jurisCategory.books)
            ...
            {rowBooks.map((book) => (
              <article key={book.id} className="relative flex flex-col items-center" itemScope itemType="https://schema.org/Book">
                ...
                <div className="w-24 sm:w-28 md:w-32 lg:w-36 h-36 sm:h-40 md:h-48 lg:h-52 relative">
                  <Link href={`/books/islamic-jurisprudence/${book.id}`} aria-label={`Read more about ${book.title}`}>
                    <Image
      ...
      <Head>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "description": "Explore our curated collection of books from various Islamic jurisprudential schools.",
            "hasPart": jurisCategory.books.map((book) => ({
              "@type": "Book",
              "name": book.title,
          ...
          <div className="flex flex-col items-center py-12 md:py-24" id="book-container">
            <div className="flex flex-col gap-28 mt-16 md:mt-24 w-full" data-category-id="islamicjurisprudence">
              ...
              <section className="w-full px-2 sm:px-4 md:px-8" aria-label="Book list">
                {renderBooksInRows(jurisCategory.books)}
              </section>
            </div>
          </div>
        ) : (
